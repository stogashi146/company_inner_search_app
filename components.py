...
def display_chat_introduction():
    with st.chat_message("assistant"):
        st.success("こんにちは。私は社内文番の情報をもとに回答する生成AIチャットボットです。サイ ドバーで利用目的を選択し、山血下部のチャット機からメッセージを送信してください。")
    ...
    if llm_response["answer"] != ct.INQUIRY_NO_MATCH_ANSWER:
        ...
        for document in llm_response["context"]:
            ...
            if "page" in document.metadata:
                ...
                # ファイル情報を表示
                st.info(f"{file_info} (ページNo.{page_number})", icon=icon)
            else:
                # 「ファイルパス」のみ
                file_info = f"{file_path}"
